<script lang="ts">
	...
	if (required !== undefined && constraints !== undefined) {
		throw new Error(
			'TextInput: Cannot provide both "required" and "constraints" props. They are mutually exclusive.'
		);
	}
	...
	let inputClass = $derived(
		cn(
			'input block h-10 w-full p-2.5 shadow-sm',
			leadAdornment
				? 'rounded-s-0 rounded-e-md focus-visible:outline-none focus-visible:border-l-0'
				: undefined,
			tailAdornment
				? 'rounded-s-md rounded-e-0 focus-visible:outline-none focus-visible:border-r-0'
				: undefined,
...
<div class={wrapperClass}>
	...
	{#if errors}
		<div class="text-error-700 px-1 text-xs leading-none tracking-tighter uppercase">
			{errors}
		</div>
	{/if}
</div>
