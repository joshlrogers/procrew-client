<script lang="ts">
	...
	let baseClass = $derived(
		cn('flex items-center cursor-pointer', orientation === 'horizontal' ? 'flex-col' : 'flex-row')
	);
	const buttonClass = $derived(cn(status === 'current' ? 'cursor-default' : 'cursor-pointer'));
	const wrapperClass = $derived(
		cn('flex items-center', buttonClass, orientation === 'horizontal' ? 'flex-col' : 'flex-row')
	);
</script>
...
>
	{#if description}
		<Tooltip
			text={description}
			cursor={status === 'current' ? 'default' : 'pointer'}
			triggerClass={baseClass}
		>
			<div class={stepClass}>
				<Icon {icon} />
			</div>
