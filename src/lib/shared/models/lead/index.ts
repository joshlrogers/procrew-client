...
const leadSchema = z.object({
    ...
    emailAddress: z.string().max(250, 'Email is too long.').refine((val) => !val || val === '' || z.string().email().safeParse(val).success, 'Invalid email address.').optional().nullable(),
    ...
    if (data.address) {
        const hasAddressData = data.address.addressLine1?.trim() || 
