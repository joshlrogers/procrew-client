<script lang="ts">
	...
	let { form, enhance, errors, constraints, tainted, isTainted, submitting, reset } = superForm(
		defaults(zod(QuickAddCustomerSchema)),
		{
			dataType: 'json',
			invalidateAll: false,
			...
			onUpdated: async (event) => {
				if (event.form.valid) {
					reset({
						...
						newState: { customerType: CustomerType.RESIDENTIAL }
					});
					onClose(true);
					return;
				}
...
{#if $open}
	<div use:melt={$portalled}>
		...
		></div>
		<div
			class="bg-surface-900 fixed top-1/2 left-1/2 z-50 max-w-[90vw] min-w-[400px] -translate-x-1/2 -translate-y-1/2 rounded-xl shadow-lg"
			use:melt={$content}
		>
