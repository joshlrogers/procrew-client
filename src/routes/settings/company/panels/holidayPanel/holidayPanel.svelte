<script lang="ts">
	...
	let { company }: HolidayPanelProps = $props();
	...
	let { enhance, form, reset, isTainted, tainted, delayed } = superForm<Holidays>(
		defaults({ holidays: [] }, zod(CompanyHolidaysSchema)),
		{
			dataType: 'json',
	...
	function addHoliday() {
		...
		let exists =
			$form.holidays.findIndex(
				(sh) => sh.day === companyHoliday.day && sh.month === companyHoliday.month
			) !== -1;
		let isCompanyHoliday =
			companyHolidays.findIndex(
				(sh) => sh.day === companyHoliday.day && sh.month === companyHoliday.month
			) !== -1;
		let isFederalHoliday =
			federalHolidays.findIndex(
				(sh) => sh.day === companyHoliday.day && sh.month === companyHoliday.month
			) !== -1;
	...
	function applyHolidays(currentHolidays: Holiday[]) {
		...
		for (let ix = 0; ix < currentHolidays.length; ix++) {
			let fedHolidayIndex = federalHolidays.findIndex(
				(fh) =>
	...
	function getHolidayName(holiday: Holiday) {
		...
		const preamble = isDynamic
			? `${addNumberSuffix(holiday.week)} ${holiday.dayOfWeek ? daysOfWeek[holiday.dayOfWeek] : ''} of `
			: '';
...
<form method="POST" action="?/updateHolidays" use:enhance>
	<div class="flex w-full flex-col items-center gap-4 px-4">
		<Panel title="Federal holidays" class="w-full">
			{#snippet header()}
				Federal holidays
			{/snippet}
			{#snippet content()}
				<div class="flex flex-col md:max-h-96 md:flex-wrap md:content-center">
		...
		<Panel title="Company holidays" class="w-full">
			{#snippet header()}
				Company holidays
			{/snippet}
			{#snippet content()}
				...
				<div class="mt-2 flex max-h-96 flex-col flex-wrap items-center">
